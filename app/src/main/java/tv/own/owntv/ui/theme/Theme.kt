...
/**
 * Available HanTV themes. Persisted via DataStore and selectable from Settings → Theme.
...
/** Map the resolved HanTV tokens onto a tv-material3 M3 [ColorScheme]. */
private fun schemeFrom(c: HanTVColors): ColorScheme =
    if (c.isDark) {
        darkColorScheme(
            primary = c.primary,
...
fun HanTVTheme(
    ...
    CompositionLocalProvider(
        LocalHanTVColors provides colors,
...
/** Convenience accessor: `HanTVTheme.colors.focusBorder`. */
object HanTVTheme {
    val colors: HanTVColors
        @Composable
        @ReadOnlyComposable
        get() = LocalHanTVColors.current
}
