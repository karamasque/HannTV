    ...
    Box(
        ...
        Column(Modifier.dialogPanel(width = 560.dp, padding = 28.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                if (hasImage) {
                    HanTVButton(stringResource(R.string.common_clear), onClick = onClear, style = HanTVButtonStyle.SECONDARY)
                }
                HanTVButton(stringResource(R.string.setup_from_phone), onClick = onPickRemote, style = HanTVButtonStyle.SECONDARY)
                HanTVButton(stringResource(R.string.setup_from_device), onClick = onPickLocal, modifier = Modifier.focusRequester(firstFocus))
            }
        }
    }
}
        ...
        Column(Modifier.dialogPanel(width = 560.dp, padding = 28.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            when (state) {
                ...
                is tv.own.owntv.core.companion.CompanionServerState.Failed -> {
                    ...
                    Spacer(Modifier.height(12.dp))
                    HanTVButton(stringResource(R.string.setup_try_again), onClick = { onStart(tv.own.owntv.core.companion.CompanionLink.DEFAULT_PORT) })
                }
                tv.own.owntv.core.companion.CompanionServerState.Locked -> {
                    ...
                    HanTVButton(stringResource(R.string.setup_start_again_new_pin), onClick = { onStart(tv.own.owntv.core.companion.CompanionLink.DEFAULT_PORT) })
                }
            }
            Spacer(Modifier.height(20.dp))
            HanTVButton(stringResource(R.string.common_cancel), onClick = onDismiss, style = HanTVButtonStyle.SECONDARY, modifier = Modifier.focusRequester(firstFocus))
        }
    }
}
