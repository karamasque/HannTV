 ...
 * The connection is per HanTV profile; users sign in with their own free OpenSubtitles account.
 */
@Composable
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            if (state is OpenSubtitlesViewModel.UiState.SignedIn) {
                HanTVButton(stringResource(R.string.player_subtitles_refresh), onClick = { vm.refresh() }, style = HanTVButtonStyle.SECONDARY)
            }
        }
        ...
        when (val s = state) {
            is OpenSubtitlesViewModel.UiState.SignedIn -> {
                ...
                ServiceSettingsRow(
                    icon = HanTVIcon.PERSON, title = stringResource(R.string.player_subtitles_sign_out),
            ...
            OpenSubtitlesViewModel.UiState.SignedOut -> {
                GroupLabel(stringResource(R.string.player_subtitles_account))
                ServiceSettingsRow(
                    icon = HanTVIcon.PERSON, title = stringResource(R.string.player_subtitles_sign_in),
        ...
        ServiceSettingsRow(
            icon = HanTVIcon.LANGUAGE, title = stringResource(R.string.player_subtitles_filter_title),
        ...
        if (filterEnabled) {
            ...
            ServiceSettingsRow(
                icon = HanTVIcon.LANGUAGE, title = stringResource(R.string.player_subtitles_search_language),
        ...
        ServiceSettingsRow(
            icon = HanTVIcon.DOWNLOADS, title = stringResource(R.string.player_subtitles_delete_action),
...
private fun OpenSubtitlesApiPopup(
    ...
    tv.own.owntv.ui.components.HanTVPopup(onDismissRequest = onDismiss, fontScale = .50f) {
        ...
        Column(Modifier.dialogPanel(width = 560.dp, padding = 20.dp)) {
            ...
            Spacer(Modifier.height(8.dp))
            HanTVTextField(value = key, onValueChange = onKeyChange, label = stringResource(R.string.settings_open_subtitles_api_key), placeholder = stringResource(R.string.settings_metadata_optional), modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            HanTVTextField(value = url, onValueChange = onUrlChange, label = stringResource(R.string.settings_worker_server_url), placeholder = stringResource(R.string.settings_metadata_optional), modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                HanTVButton(stringResource(R.string.settings_remove_custom_access), onRemove, style = HanTVButtonStyle.SECONDARY)
                Spacer(Modifier.weight(1f))
                HanTVButton(stringResource(R.string.common_cancel), onDismiss, style = HanTVButtonStyle.SECONDARY)
                HanTVButton(stringResource(R.string.common_save), onSave)
            }
        }
        }
    }
    ...
    tv.own.owntv.ui.components.HanTVPopup(onDismissRequest = onDismiss, fontScale = .50f) {
        Box(
            ...
            Column(Modifier.dialogPanel(width = 520.dp, padding = 20.dp)) {
                ...
                Spacer(Modifier.height(10.dp))
                Row2(
                    icon = HanTVIcon.SUBTITLE, title = stringResource(R.string.player_subtitles_stay_signed_in),
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    HanTVButton(stringResource(R.string.common_cancel), onClick = onDismiss, style = HanTVButtonStyle.SECONDARY)
                    Spacer(Modifier.weight(1f))
                    HanTVButton(stringResource(R.string.player_subtitles_sign_in), onClick = onSubmit)
                }
            }
        }
    }
