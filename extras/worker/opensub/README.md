# HanTV OpenSubtitles proxy (Cloudflare Worker)
...
for a few hours. Users still sign in with their **own** OpenSubtitles account inside HanTV; only
the app's consumer key lives here, so it never ships in the APK. This is the same proxy logic
HanTV's built-in default OpenSubtitles server runs.

You can run your **own free copy** and point HanTV at it — no programming needed. A personal
