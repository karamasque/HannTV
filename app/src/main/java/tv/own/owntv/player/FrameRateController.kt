 ...
 *
 * Two independent mechanisms exist on Android and HanTV uses both:
 *
...
object FrameRateController {
    ...
    /**
     * Android TV's own **Match content frame rate** setting (API 31+), and what HanTV does with it.
     *
     * **The decision, stated plainly: the system setting wins.** When the OS is set to *Never*, HanTV
     ...
     * only*, HanTV restricts itself to modes the panel can move to without re-handshaking HDMI. Anything
     ...
     *    user has been through an HDMI handshake they disliked and told the platform to stop. HanTV's is
     ...
     *    this check HanTV would be doing through `preferredDisplayModeId` precisely what the platform
     ...
     *    own display settings, and turning it back to *Always* restores HanTV's full behaviour with no
     *    change needed here.
     *
