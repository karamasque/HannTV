<p align="center">
  <img src="extras/logo.png" alt="HanTV" width="360">
</p>
...
<p align="center">
  <a href="https://github.com/ahXN00/HanTV/actions/workflows/android.yml">
    <img alt="Android CI" src="https://github.com/ahXN00/HanTV/actions/workflows/android.yml/badge.svg">
  </a>
</p>

---
HanTV is a native **Android TV** IPTV **player**, built with Kotlin and Jetpack Compose for TV. It
...
> ⚠️ HanTV ships **no** channels, playlists, subscriptions or streams. You are responsible for the
> sources you add.
...
- HanTV measures how many streams your provider allows and warns before it refuses one
...
- **Set up from another device** — a new box copies everything from the HanTV device you already
...
library, shared with the [**HanTV Mobile**](https://github.com/ahXN00/HanTV_Mobile) app for phones
and tablets.
...
HanTV_Core/  (separate repo, published as tv.own.owntv:core / :player-core)
...
HanTV_Mobile/  (separate repo, the phone and tablet app on the same core)
```

**Related repositories**

- 📱 **[HanTV Mobile](https://github.com/ahXN00/HanTV_Mobile)** — the phone and tablet app, built on
  the same core.
- 🧩 **[HanTV Core](https://github.com/ahXN00/HanTV_Core)** — the shared engine both apps run on.

## 📚 Docs (`extras/`)

- 📖 **[User Guide](extras/USER_GUIDE.md)** — every feature and where to find it.
- 📄 **[Master Product Brief](extras/HanTV_Master_Product_Brief.md)** — the full as-built feature and
  architecture reference.
- 📺 **[Player design reference](extras/player.html)** — an interactive mockup of the player UI.

## 📥 Installing (Fire TV / Android TV)

Grab the signed APK from the [**latest release**](https://github.com/ahXN00/HanTV/releases/latest).
This link always points at the newest build:

```
https://github.com/ahXN00/HanTV/releases/latest/download/HanTV.apk
```
...
- **Android TV / Google TV** — Downloader is on Google Play too, so the same code works. Or sideload
  with *Send files to TV*, a USB drive, or `adb install HanTV.apk`.

> Install only from this repository's Releases. Each release ships **two APKs**:
> `HanTV.apk` (arm — every real TV box, and what the Downloader code fetches) and
> `HanTV-x86_64-vX.X.X.apk` (emulators and rare Intel boxes).
...
[HanTV_Core](https://github.com/ahXN00/HanTV_Core) repository and is pulled from GitHub Packages,
which always asks who you are. Without it, Gradle sync fails with a `401`.

1. **Get the code** — `git clone https://github.com/ahXN00/HanTV.git` (or download the ZIP).
...
## Help translate HanTV

If your language is already available, contribute interface translations across HanTV's six Android resource components on [Hosted Weblate](https://hosted.weblate.org/projects/owntv/). If it is not listed, [open a language request ticket](https://github.com/ahXN00/HanTV/issues/new?template=feature_request.yml&title=%5BLanguage%5D%20Add%20) first. A maintainer will review the request, register the locale, and prepare its base translation files on Hosted Weblate. Once the language appears on Hosted Weblate, you can start translating it there. The strings themselves live in [HanTV's core library repository](https://github.com/ahXN00/HanTV_Core), together with the language contributor guide covering identifiers, validation, and promotion policy.
...
HanTV speaks 26 languages because people translate it on
...
HanTV is a media **player** only. It ships with no channels, playlists, subscriptions, or content, and
...
In short: you're free to use, study, modify, and redistribute HanTV, including commercially — but any
...
<sub>HanTV is an open-source, player-only project, built with the help of AI.</sub>
