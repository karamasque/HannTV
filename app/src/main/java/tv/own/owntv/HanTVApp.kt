...
class HanTVApp : Application(), SingletonImageLoader.Factory, androidx.work.Configuration.Provider {
    ...
    override fun onCreate() {
        Perf.begin() // zero-point for the HanTVPerf startup timeline (adb logcat -s HanTVPerf)
        super.onCreate()
        ...
        tv.own.owntv.core.CoreBuildInfo.releaseRepo = "karamasque/HannTV"
    ...
    override fun onTrimMemory(level: Int) {
        ...
        if (level >= TRIM_MEMORY_RUNNING_LOW) {
            ...
            runCatching { GlobalContext.getOrNull()?.getOrNull<tv.own.owntv.player.HanTVPlayer>()?.onTrimMemory() }
        }
    }
}
