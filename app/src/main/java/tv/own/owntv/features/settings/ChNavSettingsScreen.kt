...
import tv.own.owntv.ui.components.HanTVPopup
        ...
        ChNavDialog.ACTION -> pendingBinding?.let { binding ->
            ...
            PickerDialog(
                ...
                leadingIcons = buildMap {
                    if (editingExisting) put(String(), HanTVIcon.CLOSE)
...
private fun RemoteButtonCapturePopup(
    ...
    HanTVPopup(onDismissRequest = onDismiss, fontScale = .60f) {
        PopupFontTheme {
...
private fun remoteButtonIcon(keyCode: Int, fallbackAction: RemoteShortcutAction): HanTVIcon = when {
    keyCode in AndroidKeyEvent.KEYCODE_0..AndroidKeyEvent.KEYCODE_9 ||
        keyCode in AndroidKeyEvent.KEYCODE_NUMPAD_0..AndroidKeyEvent.KEYCODE_NUMPAD_9 -> HanTVIcon.MORE
    else -> when (keyCode) {
        AndroidKeyEvent.KEYCODE_CHANNEL_UP -> HanTVIcon.REMOTE_CHANNEL_UP
        AndroidKeyEvent.KEYCODE_CHANNEL_DOWN -> HanTVIcon.REMOTE_CHANNEL_DOWN
        AndroidKeyEvent.KEYCODE_MEDIA_REWIND -> HanTVIcon.REWIND
        AndroidKeyEvent.KEYCODE_MEDIA_FAST_FORWARD -> HanTVIcon.FORWARD
        AndroidKeyEvent.KEYCODE_MEDIA_PREVIOUS -> HanTVIcon.SKIP_PREVIOUS
        AndroidKeyEvent.KEYCODE_MEDIA_NEXT -> HanTVIcon.SKIP_NEXT
        AndroidKeyEvent.KEYCODE_MEDIA_PLAY_PAUSE,
        AndroidKeyEvent.KEYCODE_MEDIA_PLAY,
        -> HanTVIcon.PLAY
        AndroidKeyEvent.KEYCODE_MEDIA_PAUSE -> HanTVIcon.PAUSE
        AndroidKeyEvent.KEYCODE_GUIDE -> HanTVIcon.EPG
        AndroidKeyEvent.KEYCODE_INFO -> HanTVIcon.INFO
        AndroidKeyEvent.KEYCODE_MENU -> HanTVIcon.MENU
        AndroidKeyEvent.KEYCODE_CAPTIONS -> HanTVIcon.SUBTITLE
...
private fun remoteActionIcon(action: RemoteShortcutAction): HanTVIcon = when (action) {
    RemoteShortcutAction.OPEN_HOME -> HanTVIcon.HOME
    RemoteShortcutAction.OPEN_LIVE_TV, RemoteShortcutAction.RETURN_TO_LIVE -> HanTVIcon.LIVE_TV
    RemoteShortcutAction.OPEN_MOVIES -> HanTVIcon.MOVIES
    RemoteShortcutAction.OPEN_SERIES -> HanTVIcon.SERIES
    RemoteShortcutAction.OPEN_DOWNLOADS -> HanTVIcon.DOWNLOADS
    RemoteShortcutAction.OPEN_GUIDE -> HanTVIcon.EPG
    RemoteShortcutAction.OPEN_SEARCH -> HanTVIcon.SEARCH
    RemoteShortcutAction.OPEN_SETTINGS -> HanTVIcon.SETTINGS
    RemoteShortcutAction.OPEN_PROFILE_SWITCHER -> HanTVIcon.PERSON
    RemoteShortcutAction.OPEN_PLAYLIST_SWITCHER -> HanTVIcon.PLAYLIST
    RemoteShortcutAction.CONTINUE_LAST_WATCHED, RemoteShortcutAction.PLAY_PAUSE -> HanTVIcon.PLAY
    RemoteShortcutAction.FOCUS_NOW_PLAYING -> HanTVIcon.FOCUS_HIGHLIGHT
    RemoteShortcutAction.EXPAND_NOW_PLAYING -> HanTVIcon.EXPAND
    RemoteShortcutAction.ENTER_MINI_PLAYER -> HanTVIcon.PIP
    RemoteShortcutAction.ENTER_AUDIO_MODE, RemoteShortcutAction.OPEN_AUDIO_CONTROLS -> HanTVIcon.AUDIO
    RemoteShortcutAction.PAGE_TOWARD_FIRST, RemoteShortcutAction.JUMP_TO_FIRST -> HanTVIcon.SKIP_PREVIOUS
    RemoteShortcutAction.PAGE_TOWARD_LAST, RemoteShortcutAction.JUMP_TO_LAST -> HanTVIcon.SKIP_NEXT
    RemoteShortcutAction.OPEN_SUBTITLE_CONTROLS -> HanTVIcon.SUBTITLE
    RemoteShortcutAction.OPEN_ASPECT_CONTROLS -> HanTVIcon.ASPECT
    RemoteShortcutAction.TOGGLE_PLAYBACK_INFO -> HanTVIcon.INFO
}
