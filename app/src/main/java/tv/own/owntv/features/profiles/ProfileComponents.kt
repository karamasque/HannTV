...
import tv.own.owntv.ui.components.HanTVAvatars
...
private fun PinDialogBody(title: String, onSubmit: (String) -> Unit, onDismiss: () -> Unit, compact: Boolean) {
    ...
    ProfileScrim(onDismiss, width = if (compact) 290.dp else 480.dp, padding = if (compact) 16.dp else 28.dp) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            HanTVButton(
                stringResource(R.string.common_cancel), onClick = onDismiss, style = HanTVButtonStyle.SECONDARY,
...
internal fun ProfileEditorDialog(
    ...
    ProfileScrim(onDismiss) {
        ...
        HanTVTextField(name, { name = it }, label = stringResource(R.string.profiles_name), placeholder = stringResource(R.string.profiles_name_hint), modifier = Modifier.fillMaxWidth().focusRequester(focus))
        if (nameTaken) {
        ...
        LazyRow(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            items((-1 until HanTVAvatars.COUNT).toList()) { id -> // Phase 7 — includes "no avatar" (-1)
                FocusableSurface(
                    onClick = { avatarId = id },
                    ...
                    HanTVAvatar(avatarId = id, modifier = Modifier.size(48.dp))
                }
            }
        }
        Spacer(Modifier.height(16.dp))
