...
| `ui/components/HanTVIcon.kt` | EDIT | Add `LANGUAGE` enum entry (translate icon - "A" with lines, Google Material standard) |
...
### Step 1. Add LANGUAGE icon to HanTVIcon enum

Add `LANGUAGE` to `HanTVIcon` enum at `ui/components/HanTVIcon.kt:21-27`. Draw a translate-style glyph (A with lines) via Canvas path - Google's standard language/translate icon. All existing icons are custom Canvas-drawn paths.
...
```kotlin
SettingsRow(
    tone = TileTone.PRIMARY, icon = HanTVIcon.LANGUAGE,
...
SettingsSearchEntry(
    ...
    HanTVIcon.LANGUAGE, TileTone.PRIMARY,
    chip = languageChipText(currentTag),
    chipTone = TileTone.PRIMARY,
