    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            FocusableSurface(onClick = onBack, modifier = Modifier.size(44.dp), shape = RoundedCornerShape(14.dp), contentAlignment = Alignment.Center, surface = GlassSurface.CARDS) { _ ->
                HanTVIcon(HanTVIcon.BACK, tint = colors.onSurface, modifier = Modifier.size(20.dp))
            }
            ...
            if (liveNow in state.windowStart..state.windowEnd) {
                HanTVButton(stringResource(R.string.content_epg_jump_now), onClick = jumpToNow, icon = HanTVIcon.HISTORY, style = HanTVButtonStyle.SECONDARY)
            }
            ...
            if (guideCategories.isNotEmpty()) {
                ...
                HanTVButton(stringResource(R.string.content_epg_category_button, catLabel), onClick = { showCategoryPicker = true }, icon = HanTVIcon.MENU, style = HanTVButtonStyle.SECONDARY)
                Spacer(Modifier.width(12.dp))
            }
            HanTVButton(stringResource(R.string.content_epg_sort_button, sortLabel), onClick = vm::cycleGuideSort, icon = HanTVIcon.SORT, style = HanTVButtonStyle.SECONDARY)
            Spacer(Modifier.width(12.dp))
            // Smart-match: auto-link channels whose tvg-id doesn't match the EPG feed, by name (#13).
            if (matching) {
                Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    HanTVSpinner(sizeDp = 20)
                ...
                HanTVButton(stringResource(R.string.content_epg_match_button), onClick = vm::autoMatchEpg, icon = HanTVIcon.EPG, style = HanTVButtonStyle.SECONDARY)
            }
        }
        ...
        when {
            state.loading -> CenterBox { HanTVSpinner(sizeDp = 56) }
            ...
            !state.hasEpgSources && state.channels.isEmpty() -> CenterBox {
                ...
                HanTVButton(stringResource(R.string.content_epg_add), onClick = onAddEpg, icon = HanTVIcon.ADD)
            }
...
private fun EpgMatchReviewDialog(
    ...
    tv.own.owntv.ui.components.HanTVPopup(onDismissRequest = onDone) {
        ...
        Column(Modifier.dialogPanel(width = 576.dp, corner = 18.dp, padding = 18.dp)) {
            ...
            Column(Modifier.width(140.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (suggestions.size > 1) {
                    HanTVButton(stringResource(R.string.content_epg_accept_all), onClick = onAcceptAll, icon = HanTVIcon.PLAY, modifier = Modifier.fillMaxWidth())
                    HanTVButton(stringResource(R.string.content_epg_skip_all), onClick = onSkipAll, style = HanTVButtonStyle.SECONDARY, modifier = Modifier.fillMaxWidth())
                }
                HanTVButton(stringResource(R.string.common_done), onClick = onDone, style = HanTVButtonStyle.SECONDARY, modifier = Modifier.fillMaxWidth())
            }
            }
        }
    }
...
private fun EpgMatchChooserDialog(
    ...
    Box(
        ...
        Column(Modifier.dialogPanel()) {
            ...
            Spacer(Modifier.height(10.dp))
            HanTVButton(stringResource(R.string.content_epg_match_button), onClick = onAuto, style = HanTVButtonStyle.SECONDARY, icon = HanTVIcon.EPG, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(10.dp))
            HanTVButton(stringResource(R.string.content_epg_pick_manually), onClick = onManual, style = HanTVButtonStyle.SECONDARY, icon = HanTVIcon.SEARCH, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(10.dp))
            HanTVButton(stringResource(R.string.content_epg_time_offset), onClick = onOffset, style = HanTVButtonStyle.SECONDARY, icon = HanTVIcon.EPG, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(16.dp))
            HanTVButton(stringResource(R.string.common_cancel), onClick = onDismiss, style = HanTVButtonStyle.SECONDARY)
        }
    }
}
