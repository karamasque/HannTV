...
Everything HanTV can do, **where to find it**, and the remote shortcuts that make it fast.
...
```
Welcome  →  Text size  →  Disclaimer  →  Set up HanTV  →  Profile
                                                            ↓
...
3. **Disclaimer** — HanTV is a player; you bring the sources.
4. **Set up HanTV** — three ways to begin: **New profile**, **Restore a backup**, or
...
8. **Import** runs, and when it finishes HanTV **offers to download the TV guide** in one press.

### 🔄 Already have HanTV on another device?
**Where:** the **Set up HanTV** screen → **From another device**
...
The spinner stays up with a live countdown and HanTV retries by itself. Don't press Retry — just
wait.
...
definition; that is the device's decision, not HanTV's.
...
HanTV measures this once, at a playlist's first sync, and warns before refusing a stream. **Re-test**
...
### 📊 Your daily share
The built-in service is shared by everyone using HanTV, so it has minute/hour/day allowances shown at
the top of the page.
...
> Below **85% zoom** HanTV warns first — very small sizes draw many more items and can exhaust a
> low-memory TV.
...
Swap favourites, history and resume points with the HanTV mobile app over your own Wi-Fi — no
...
HanTV sorts each entry by its tags:
...
| "Too many connections" | Wait — HanTV counts down and retries by itself. Don't press Retry |
| Sound but a black picture | HanTV switches engines by itself; if neither can, it says so |
