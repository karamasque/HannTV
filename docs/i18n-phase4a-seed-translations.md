...
- English in `values/` is the only language HanTV guarantees to be complete.
...
- `preserveExact`: complete brand, product, protocol, and format phrases that may appear inside
  translatable sentences, such as `HanTV`, `ExoPlayer`, `mpv`, `OpenSubtitles`, `Xtream`, and `M3U`.
...
state are informational; they do not become HanTV release gates.
