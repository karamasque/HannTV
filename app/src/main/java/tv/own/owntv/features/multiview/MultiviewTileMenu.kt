...
fun MultiviewTileMenu(
    ...
    Box(
        ...
        Column(Modifier.dialogPanel(), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            ...
            if (onAddTile != null) {
                MenuRow(stringResource(R.string.multiview_add_channel), HanTVIcon.ADD, onAddTile)
            }
            if (filled) {
                MenuRow(stringResource(R.string.multiview_tile_fullscreen), HanTVIcon.EXPAND, onFullscreen)
                MenuRow(stringResource(R.string.multiview_audio_tile), HanTVIcon.VOLUME_HIGH, onSound)
                ...
                MenuRow(
                    ...
                    icon = HanTVIcon.HEADPHONES,
                    onClick = onSoundOnly,
                )
                MenuRow(stringResource(R.string.multiview_tile_remove), HanTVIcon.CLOSE, onRemove)
            }
            MenuRow(stringResource(R.string.content_close), HanTVIcon.CLOSE, onDismiss)
        }
    }
}
...
private fun MenuRow(
    ...
    FocusableSurface(onClick = onClick, modifier = modifier.fillMaxWidth()) {
        Row(
            ...
            HanTVIcon(icon, tint = colors.onSurfaceVariant, modifier = Modifier.size(18.dp))
