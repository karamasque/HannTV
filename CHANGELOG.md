...
- **"From another device" on the very first setup screen.** Setting up a new box no longer means
  ...
  history straight over the Wi-Fi from the HanTV device you already have. Turn Sync mode on over
...
- **The user guide now describes first-run setup exactly as it happens.** It listed five screens
  where there are eight, folded the "Set up HanTV" choice into the profile step, and never mentioned
...
- **Local sync.** Settings → Data → Local sync pairs this television with the HanTV app on your phone
...
- **A deletion now stays deleted.** Unfavourite a channel here, sync, and it does not come back from
  the phone on the next sync. This is why the database gains a table on this update: HanTV now
...
### 🔌 HanTV works out how many channels your provider allows
...
HanTV now finds out by trying, **once**, when a playlist is added — before any channels are saved, so
...
- **A recording scheduled across a reboot is repaired the next time HanTV is opened.** Some
...
- **Films and shows your provider gave no artwork for now show the poster HanTV already found.** Their
...
- **A crash is written to disk the moment it happens** and survives the app being killed, so it is still
  there the next time HanTV opens. **Export** puts it at the top of the report file it saves to Downloads,
...
- **The largest single cause was the list of code HanTV asks Android to prepare ahead of time.** That
...
- **Updated to the current release of every build tool and library HanTV depends on**, including Kotlin,
...
- **Single-playlist browsing stays exactly as clean as before.** Provider labels appear only when they are
  useful; with zero or one active source, HanTV leaves the existing layouts unchanged.
...
- **New setting: Settings → Video player → Live TV → "Give up on a channel after".** A live channel
  ...
  HanTV silently worked through four different player-and-format combinations and each one had its
...
- **A wait your provider asks for is never counted against it.** When a panel answers "your account is
  busy, try again in 20 seconds", HanTV is counting that down behind the spinner on purpose. That time
...
- **Catching up with the present hands over to live.** If the programme that follows is the one on the
  air right now, HanTV tunes the live channel instead of asking the provider for a recording that is
...
- **Embedded TMDB trailers do less work on lower-memory TVs.** The in-app YouTube surface no longer
  builds its own controls, captions, annotations or related-video UI behind HanTV's controls, and the
...
- **Rewinding a channel with no recording available now returns to live.** If the provider had nothing
  to serve, HanTV kept showing "behind live" and a rewind counter over a picture that had never left
...
- **Your audio sync survives a stream reconnecting.** A channel that dropped and recovered — or that
  ...
  had not asked HanTV to remember was lost outright. Zoom and volume already survived a reconnect;
...
- **Auto frame rate now respects your TV's own "Match content frame rate" setting.** If you had
  turned frame-rate matching off in Android TV's display settings, HanTV changed the display mode
  anyway — it was using an older method that the system does not police. Set to **Never**, HanTV now
...
- **"App not installed" when updating from inside HanTV.** The app downloaded the new version and
  ...
  "App not installed" and no way to tell why. HanTV now checks there is room before it starts, checks
  the download arrived whole, and checks Android can actually read the file as HanTV before offering
...
- **An update that the system refuses now says why.** Installing goes through Android's own installer
  service instead of opening the downloaded file as a document, which means HanTV can read back the
...
- **A frozen live channel moves to the other player about twice as fast.** The wait before handing a
  ...
  already failed.
- **The offline warning now appears when the internet is actually unreachable.** HanTV only checked
...
- **Track menus stop hunting for tracks that will never arrive.** Opening the audio or subtitle menu on
  a stream that has none — a radio channel, most often — left HanTV re-checking three times a second
...
- **Kids mode now hides adult provider categories and their items throughout HanTV.** The same
...
### 🚀 Start HanTV on a specific Live TV channel
...
- **Unavailable or restricted startup channels fail safely.** Hidden channels, channels blocked by
  ...
  longer exists, HanTV opens Home and explains what happened.
...
- **Both services now support advanced custom access.** TMDB and OpenSubtitles each accept a personal API key or Worker/server URL in a compact popup; a URL takes priority over a key, while leaving both blank uses HanTV's built-in service. OpenSubtitles custom access is included in Backup & Restore.
...
- **Android's automatic backup is switched off.** The app was letting Android copy its own database
  ...
  secrets are left out. **Settings → Backup & Restore is now the only way HanTV data moves between
...
- **The two "only" choices stop the automatic switch between players.** Handing a channel from one
  ...
  only: no switch after a decode failure, an account-busy refusal or anything HanTV worked out for
...
- **The compatibility-mode button in the player now stays where you put it.** Switching a live channel
  back to ExoPlayer by hand could be undone by HanTV about two seconds later, over and over, on TVs
...
- **HanTV can now play channels and films protected with Widevine or ClearKey.** Some providers —
  including self-hosted setups such as JioTV-Go — publish MPEG-DASH channels that are locked, with the
  unlock address written into the playlist. Until now HanTV threw that address away while reading the
...
- **Nothing to set up, and nothing to buy.** The unlocking is done by the component already built into
  every Android TV and Fire TV device; HanTV just has to ask it. There is no key to enter, no account,
...
- **Two things are worth knowing.** Older or cheaper TV boxes may only be allowed to play protected
  channels in standard definition — that is decided by the device, not by HanTV. And a protected item
  always plays inside HanTV even if you have chosen an external player, because no external player can
...
- **OpenSubtitles sign-in now tries the server's other address.** OpenSubtitles is reached through an
  ...
  the first attempt failed and HanTV gave up instantly — so signing in was impossible while everything
...
- **“Audio only” no longer flashes while an ordinary TV channel is starting.** Some providers announce
  ...
  and removed the message only when the picture arrived. HanTV now waits for a ready stream to remain
...
- **Downloaded subtitles are now told apart, and subtitle timing changes the one you selected.**
  ...
  three identical rows — and because that name was also how HanTV identified them internally, it
...
- **A music-only movie no longer fails at about six seconds.** HanTV used to watch for "loaded, but no
...
- **Each device now has its own daily allowance, and can see it.** The built-in metadata service is
  shared by everyone using HanTV, and previously nothing stopped a single device consuming all of it.
...
- **Deinterlacing** (Settings → Video player, Off by default) — smooths the comb-shaped lines some old
  interlaced channels show on movement. It only has an effect when HanTV draws the picture itself
...
- **Your resume position is saved reliably again — and never lands in someone else's profile.** HanTV
...
- **Catch-up you watch inside HanTV now appears in History.** Only the external-player path recorded it,
...
- **Audio focus is released while you are paused,** so other apps are no longer left ducked for as long
  as the player sits paused, and a volume change made while HanTV was ducked by another app is kept
...
- **One channel refusing a request no longer sends a whole provider down the slow path.** When a panel
  answered "too many requests" during quick channel-hopping, HanTV read that as "this channel has no
  ...
  a lesson about the format, and what HanTV *does* learn about a channel now stays with that channel.
...
### 🌍 HanTV in 24 languages — choose before setup, change anytime (community PR #108 by @codeVerine)

- **HanTV's complete interface is now available in English plus 23 fully translated languages:** Arabic,
...
- **A fresh installation asks for the app language before Get Started.** The selected language and its
  description are shown in a compact TV-friendly control; the full list opens in HanTV's standard popup,
  with isolated D-pad focus, scrolling, and focus restored after selection. The welcome, disclaimer, setup
  choice, and add-playlist pages now share a modern HanTV design with a subtle animated teal glow and ring.
...
- **Home can now open with a large Now Trending showcase built from the current TMDB movie and TV charts.**
  HanTV publishes up to 10 entries, only after matching TMDB's candidates to exact, playable rows from the
...
- **Existing data upgrades in place.** Room migrations add the atomic Trending snapshot, item and indexed
  ...
  All new interface text is translated across every packaged HanTV language.
...
### ⚡ Faster cold start — HanTV now tells Android what it needs before it needs it

- **Opening the app from cold is about twice as fast.** HanTV now ships a startup profile: a list of
...
- **The speed-up arrives on Android's schedule rather than immediately.** HanTV hands the profile to
...
- **Your Movies & Series player choice is no longer overridden after a few failed streams.** Once three
  items in a row had fallen back from ExoPlayer to mpv, HanTV switched the rest of the session to mpv —
...
- **Switching audio language mid-playback no longer makes the sound stutter.** On soundtracks the TV
  ...
  affected. HanTV now re-primes the audio output once when it changes a passed-through track, costing
  a brief re-buffer; soundtracks HanTV decodes itself are untouched. Live TV gets the same fix, applied
...
- **The stream information overlay now says correctly when your TV is decoding the audio.** Passthrough
  ...
  the TV untouched was described as decoded by HanTV.
...
- **Stalker imports no longer lose a whole category when the portal drops the connection.** HanTV
...
- **The stereo-fallback notice no longer remains over the next video.** The surround safety net still
  explains when a TV audio output fails and HanTV switches the item to stereo, but repeated notices now
...
- **Surround sound no longer switches itself off because of a small timing gap inside a file.** The
  ...
  itself and carries on — but HanTV read it as a failure, dropped the whole session to stereo and
...
- **Changing the interface language in Settings now keeps focus on the selected language.** HanTV
...
- **Posters and other remote artwork no longer stay blank on TVs with a broken IPv6 route.** Image
  ...
  though they cannot actually reach it. HanTV stopped after that first failed address while the TV's
...
- **A channel that is briefly full now waits and starts by itself, instead of dropping you on the error
  ...
  seconds" — typically because the channel you just left is still counted as open. HanTV treated that as a
...
- **Live channels that switch themselves to the compatibility player play again instead of turning
  black — this fixes a v4.1.7 regression.** When HanTV decides by itself that a channel needs the
...
- **A provider that doesn't serve HLS at all is now recognised after three channels.** Before, every
  ...
  do HLS".
- **Streams that VLC plays but HanTV didn't now get one more attempt with error tolerance turned on.**
...
- **Turning "Prefer HLS" off could stop every channel playing on the standard player until the app was
  ...
  HanTV that the provider serves HLS behind its `.ts` addresses — a lesson that applies to the whole
...
- **Playback reports now export to the TV's public Download folder.** The report is written as
  ...
  is visible to normal file managers and easy to copy without ADB. HanTV creates the Download folder if
...
- **Playlists whose provider blocks media players now play.** Some providers sit behind a protection
  ...
  players, which looked exactly like a dead provider. HanTV now notices that kind of refusal and asks once
...
- **When a provider explains why it refused a channel, that explanation is now shown on the error
  ...
  Stop one of your active streams before opening a new channel." — and HanTV used to throw it away and
...
- **A channel is no longer sent looking for an address that doesn't exist when the provider is simply
  busy.** If a provider refused because the account already had a stream open, HanTV treated it as a
...
- **The setting is now three choices instead of an on/off switch: Auto (new default), Stereo only, and
  ...
  receiver to decode; **Stereo only** decodes it inside HanTV and sends plain stereo, which is the right
...
- **The setting now actually applies to Live TV.** It only ever reached the compatibility player, and
  ...
  what you changed.
- **New safety net: if your TV or soundbar accepts the sound and then doesn't play it, HanTV notices and
...
- **Stream info now tells you what the audio is actually doing.** A new **Audio out** row shows whether
  your TV/receiver is decoding the sound (passthrough) or HanTV is, whether surround is currently
...
- **A one-time suggestion when it would help.** If a channel judders because its frame rate doesn't
  divide into your TV's refresh rate, and your TV really has a better mode, HanTV offers to turn Auto
...
- **Play/pause, next and previous from the remote, a headset or voice now reach the player**, through a
  ...
  offered for films and episodes only. With the player closed, those keys do nothing to HanTV.
...
- **On providers that allow one stream at a time, the preview pane now says so** instead of sitting
  blank, which read as a broken channel. HanTV also reads the limit straight from the provider's account
...
- **Prefer HLS no longer breaks the odd channel that has no HLS version.** The setting asks for every
  ...
  before the setting was turned on. HanTV now spots that and reopens just that channel in its original
...
- **A channel that loads video but never starts playing is now spotted in seconds.** Some streams — traced
  ...
  moment. Everything HanTV watched for was armed by the first frame, which is precisely what never arrived,
...
- **A channel that can't fill the "Pre-buffer" opens anyway instead of spinning.** The setting asks the
  ...
  a second, which no stall detection could see because every restart looked like a recovery. HanTV now
...
- **Settings → Network → DNS** is a new screen, a sibling to the existing Proxy screen. Configure a
  custom DNS server and all HanTV domain lookups — playlist sync, Xtream/Stalker API, EPG, images,
...
- **Auto frame rate could still blank the TV when the setting was Off.** Media3/ExoPlayer and mpv each
  had their own surface-level frame-rate request in addition to HanTV's window-level controller, so
...
- The date comes from your provider. **Xtream playlists** carry a real added/last-modified date for
  every movie and show, and that is what's used. **Stalker portals** carry one too, and HanTV now reads
...
- Titles with no date always sort last, so a mixed catalog puts everything HanTV knows a date for
...
- The two are genuinely independent: newest seasons first with oldest episodes first is a valid
  combination, and HanTV keeps it.
...
- **Type a channel number while watching Live TV full screen** and HanTV tunes straight to it, the way a
...
- **Xtream sources have a new "Prefer HLS for Live TV" option.** Xtream panels can serve a live channel
  two ways: as a raw MPEG-TS stream, or as an HLS playlist. HanTV asked for MPEG-TS; some panels are
...
- **Your background image travels with the backup.** The Glass Effect wallpaper lives in HanTV's own
...
- Sending a backup between TVs over Wi‑Fi works exactly as before and transfers `.own` files, in both
  directions.
- Note for anyone downgrading: an older HanTV build cannot read a `.own` file. Keep a `.json` backup if
...
- **Settings → Video player → Auto frame rate** *(on)*: in full screen, HanTV now asks the TV to switch
...
- **Fixed: auto frame rate did nothing on Android 10 and older devices** — including Fire TV Stick 4K /
  4K Max on Fire OS 7. HanTV only used `Surface.setFrameRate()`, which doesn't exist before Android 11,
...
- **Settings → Playback → Catch-up** (renamed from *Catch-up time*) gained **Play catch-up in**:
  - **HanTV player** *(default — unchanged behaviour)*
...
- Sent to an external player, a recording loses the HanTV HUD, resume position and the engine toggle.
...
- **The single external-player switch is now three.** *Settings → Video Player Settings → External
  ...
  live channels to VLC while keeping movies in HanTV (or the other way around). Downloads follow the
...
- **Fixed: "no external player found" for live channels.** Live streams ending in `.ts` or `.m3u8` were
  ...
  with both installed. HanTV now widens the type until a player accepts it.

### 🛡️ Your library can no longer be wiped by a database problem

- **A schema problem no longer deletes everything.** HanTV used to be built to drop every table and
...
- **A half-finished sync can no longer delete your catalog.** If a provider returns a short or broken
  response, HanTV now refuses to remove titles when that would delete more than half of a source, and
...
- **Backups are written safely.** A backup is written to a temporary file, flushed, and only then
  ...
  is unreadable. An interrupted restore is no longer silent: HanTV notices on the next start and tells
  you.
...
- **Reconnect keeps trying.** The retry ladder is 1.5 / 3 / 6 / 10 / 15 s and then holds at 15 s
  ...
  for a minute.
- **Outages recover by themselves.** If the network drops long enough for the channel to stop, HanTV
...
- **Raw MPEG-TS channels no longer drop the connection every 10–15 seconds.** A live MPEG-TS channel is
  ...
  buffered — and HanTV's buffer settings left that socket idle long enough for providers and middleboxes
...
- **Downloads keep running when you leave the app.** Transfers moved to a proper foreground background
  worker with a notification, so Android no longer kills them the moment HanTV goes to the background.
...
- **A branded splash instead of a blank window** on cold start, held until HanTV actually knows which
...
- **A fresh install is no longer slow for the first few days.** Android normally leaves most of an
  ...
  sideloaded apps like HanTV hardest, since every new release starts that over. The APK now ships a
...
- **Remote restore.** **Settings → Backup & Restore → Restore from another device**, and the same
  ...
  opens the page and uploads an HanTV backup JSON straight to the TV; when it arrives it flows into
...
- **Search OpenSubtitles from the player.** For any movie or series episode, open **Subtitles →
  ...
  profile and title. Never automatic: HanTV only searches or downloads when you ask.
- **OpenSubtitles account, per profile.** Sign in from **Settings → Video Player → Subtitles →
  ...
  Each HanTV profile connects its own account; the allowance display shows the provider's own
...
- **Local subtitle files — no account, no internet.** **ADD SUBTITLES → Select local subtitle file**
  opens HanTV's TV-safe file browser for `.srt` / `.ass` / `.ssa` / `.vtt` / `.webvtt` files (USB or
  internal storage). Non-UTF-8 files (Windows-1256 Arabic, Windows-1252, ISO-8859…) are detected and
  converted automatically so they render correctly, and HanTV keeps a managed copy so the subtitle
...
- **Smart caching, quota-friendly.** Downloads are cached on the device and deduped: re-picking a
  ...
  **HanTV Downloads** — offline, with the OpenSubtitles moviehash silently sharpening online matches
...
- *Privacy:* the OpenSubtitles API key lives only in an HanTV-run Cloudflare Worker (like the TMDB
...
- **Remote setup shows a QR code, a URL, and a one-time PIN.** Open the server on the TV, then on a
  ...
  PIN shown on the TV, then shows an **HanTV-styled form** with Xtream / M3U / Stalker tabs. Fill it,
...
- *Core idea from **@zarga03** (PR #66)* — reimplemented and hardened for HanTV: added M3U support and
...
- **One-click "Grant full storage access."** The file/folder picker (download folder, local M3U
  import, backup) now has a single grant action that opens **HanTV's own app-settings page**, where
...
- **EPG match now falls back to a network re-sync when the cache has no data for it.** After
  matching a channel, HanTV fills its programmes from the cached XMLTV without a network call — but
...
- **Live TV "Now" no longer shows a future programme** (#68). For channels without a configured guide,
  HanTV falls back to the provider's short-EPG. When that data had a gap around the current moment, the
...
- **Smaller downloads — split ABI builds.** Releases now ship a single **arm APK** (`HanTV.apk` /
  `HanTV-vX.X.X.apk`, `arm64-v8a` + `armeabi-v7a` — for all real Fire TV / Android TV devices, and what
  the Downloader code fetches) plus a separate **`HanTV-x86_64-vX.X.X.apk`** (for emulators / rare Intel
...
- **CI dev builds are now release builds.** Every push now produces a release-signed, R8-shrunk
  `HanTV-dev-<sha>.apk` artifact (previously debug), versioned `99.99.99` so it installs straight
...
- **Set TMDB name (long-press)** — manual override for titles the matcher still gets wrong: type the exact
  TMDB title (and optional year) and HanTV re-searches under that name, on Movies and Series. The override
...
- **Self-hostable metadata server** — the caching-proxy Worker source now ships in `worker/` with a README,
  so anyone can deploy their own and point HanTV at it.
...
- HanTV has moved from the **MIT License** to the **GNU General Public License v3.0 (GPLv3)**. HanTV
...
- **Global HTTP proxy support** — **Settings → Network → Proxy** lets you route all HanTV traffic
...
- **Home screen with Continue Watching** — a new **Home** tab opens to a hero carousel of your partially‑watched
  ...
  **Favourite Channels** rail. On **stock Android TV** launchers it also feeds the system **"Continue Watching"**
  ...
  contributing this entire Home screen feature ([PR #31](https://github.com/ahXN00/HanTV/pull/31)).**
...
- **One-tap guide sync after adding a playlist** — after importing a playlist (first-run setup or Settings →
  Playlists), HanTV now asks **"Sync the TV guide now?"** if the playlist has a guide feed. **Sync now** shows
...
- **Startup focus rests on the nav** — on a cold start (or switching to the Home tab) focus now stays on the
  **Home item in the sidebar** instead of being pulled into the content; it only jumps into the hero when you
  ...
  [PR #32](https://github.com/ahXN00/HanTV/pull/32).)
...
- **Surround sound is now off by default (opt-in), with a safety net** — v3.1.1's multichannel-LPCM surround
  ...
  for a real 5.1/7.1 receiver. When on, HanTV pins a widely-compatible **16-bit / 48 kHz** output and, if it
...
- **Surround sound no longer stutters video** — the v3.1.0 *Surround passthrough* toggle bit-streamed raw
  ...
  noticeable on 4K). The setting is now simply **Settings → Surround sound** (on by default): HanTV
...
- **M3U live channels that wouldn't play now work** — after v3.1.0's faster channel-zapping, some live
  ...
  couldn't open those streams), while Xtream live was unaffected. HanTV now uses the full probe for
...
- **Episodes now appear for every Xtream series** — some providers return a series' episode data in a
  different JSON shape, which HanTV didn't read, so those shows opened with **no episodes** (they worked in
...
- **Hide/show a whole range of categories at once** — in **Settings → Customize**, long-press a category's
  ...
  (by @dan-maloney, #20)
- **Auto-play next episode** — when an episode finishes, HanTV automatically starts the next one, and
...
- **Surround sound passthrough** — a new **Settings → Surround passthrough** toggle sends **Dolby
  ...
  mixing down to stereo. HanTV only passes through the formats your audio output reports it can handle,
...
- **Faster channel zapping** — live channels and HLS streams now start with a **trimmed stream probe**,
  ...
  stream's audio (rare, on sparse feeds), HanTV automatically **re-probes that channel in full** so it
...
- **Live channels that dropped out every few seconds now play continuously** — some live servers close the
  connection on a schedule (common with 4K feeds); HanTV now **reconnects automatically at the stream level**
...
- **Installs on non-TV devices now** — HanTV required the Android **TV (leanback)** feature, so it
...
- **EPG sources that failed with a "protocol error" now load** — some EPG/host CDNs have flaky HTTP/2
  ...
  HanTV now uses HTTP/1.1 for its downloads, which those servers handle reliably. (#17)
...
> 💬 **Join us on Telegram** — **Settings → About** now shows the HanTV **Telegram group** link with a
...
- **Huge playlists import fully again** — some Xtream panels cut off very large movie/series lists
  ...
  unable to sign in. Now, if the bulk list truncates, HanTV automatically **fetches it category by
...
- **Easier Fire TV install** — releases now also publish a stable `HanTV.apk` so a fixed
  `…/releases/latest/download/HanTV.apk` link always serves the newest signed build. Fire TV users
...
- **In-app updates** — HanTV updates itself straight from GitHub Releases: automatic check shortly
