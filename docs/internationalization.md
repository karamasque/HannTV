# Internationalization & Localization for HanTV
...
5. **Localises all HanTV-authored Android TV launcher prose**, but gives the low-usage legacy launcher no special locale-change republish path. Existing entries update on the next normal publisher run.
...
The launcher publisher is a real final renderer even though the Android TV system displays its output. Localise all HanTV-authored launcher prose:
...
**Priority decision:** the legacy Android TV launcher is used by a small minority of HanTV users, and the current publisher does not target the Google TV launcher. Do not introduce a special locale-change observer, coordination worker or immediate republish operation. Existing entries may remain in the previous language until the next normal publisher run. When that run occurs, every newly written HanTV-authored field uses the current locale.
...
Everything v1 said about the mirror race (`importSettings` bypassing the mirror, the `HanTVApp.onCreate` repair collector, the force-stop window) is deleted. There is no mirror, so there is no window and no collector. This is the single largest complexity reduction in v2.
...
**Application wrapping is retained deliberately, on a platform ground that survives v2.** v1's stated reason was partly that Koin singletons and Workers resolved strings against the injected Application context (`HanTVApp.kt:43` passes `androidContext(this@HanTVApp)`). Under v2 they no longer resolve strings at all, so that reason is gone. The *other* reason is not: `ConfigurationController.handleConfigurationChangedInner` unconditionally calls `updateLocaleListFromAppContext(...)`, which **resets `Locale.getDefault()` to the device locale on every system configuration change** unless the Application context carries your locale. That affects `java.text`, `java.time`, `String.format` and every `Locale.getDefault()` reader in the process, none of which are Compose.

The Application context carries the **startup** locale and does not mutate after a same-script in-session switch. Therefore wrapping at startup is necessary but not sufficient. `HanTVApp.onConfigurationChanged(newConfig)` must re-read `LocaleStore.readBlocking()` and call `AppLocale.applyGlobally(tag)` after `super.onConfigurationChanged(newConfig)`. This restores the current selected locale after the framework's process-level reset. When `tag == ""`, `applyGlobally` resolves the new device locale list through the system-locale rule above.
...
Other modified files in 0b: `HanTVApp.kt`, `MainActivity.kt`, `features/settings/data/SettingsRepository.kt` (locale export/import wiring to `LocaleStore`).
...
- It strips **library** locale folders at merge time. appcompat 1.7.1 alone contributes ~85 locale
  ...
  removes them before HanTV translations are added. The Phase 0 baseline is recorded; later measurements
...
| Existing system-toast and chooser presentation sites in `HanTVPlayer` / `ExternalPlayerLauncher` | Final Android UI renderers; preserve existing behavior without generic plumbing |
| Narrow launcher presentation helper used by `TvHomeRepository` / `LauncherRecommendationPlanner` | Final legacy Android TV launcher publisher; resolves only HanTV-authored prose |
...
`Sidebar`, `TopBar`, `HanTVShell`, and `components/SettingsScreen.kt` (1,997 lines, 68 literals, the single largest file), plus `ShellViewModel.MainSection` enum labels. Categories 1 and 3. `MainSection` is a UI-layer enum, so `@StringRes` display metadata on the enum is acceptable here.
...
`PlayerHud`, `HanTVPlayer` (~35 error strings), `ExoSubtitleEngine`, `LivePreviewEngine`, `PlayerDiagnostics`, `AudioNowPlayingBar`, `ZoomMode`, `MiniPlayerPosition`, and `core/player/ExternalPlayerLauncher.kt`. Categories 1, 3 and 8. `ZoomMode` and `MiniPlayerPosition` are UI enums and may carry `@StringRes` directly. Preserve the existing system-toast and chooser behavior; resolve fixed HanTV messages at display time through the current-locale context. Do not add Compose event plumbing solely for this migration.
...
4. Set the TV language to French, then choose HanTV's "System default".
5. Confirm HanTV immediately changes to French and persists `""`, not `"fr"`.
6. Change the TV language to Spanish while HanTV follows system.
7. Confirm the next configuration callback applies Spanish.
8. Repeat with a TV language outside the 24 supported languages and confirm HanTV falls back to source English.
...
5. Trigger an existing `HanTVPlayer` system toast and an `ExternalPlayerLauncher` fallback/chooser; confirm fixed HanTV text is German.
...
7. Let the normal legacy Android TV launcher publisher run; confirm newly published HanTV-authored row, description, media type, season and episode prose is German.
...
- A same-script custom-locale switch followed by `HanTVApp.onConfigurationChanged` re-applies the current store value rather than the startup locale.
...
- Companion HTML, notifications, existing system toasts/chooser UI and launcher publication resolve fixed HanTV text using the current locale at render time.
...
| 15 | Launcher localisation was used to justify broad non-Compose string resolution | Localise all HanTV-authored launcher prose through a narrow publisher-time renderer. Because the legacy Android TV launcher has low usage and does not cover Google TV, do not add a special locale-change republish path | Launcher section, Phase 1 batch 11 |
...
- Reapplying the current `LocaleStore` value from `HanTVApp.onConfigurationChanged`.
