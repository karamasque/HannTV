...
fun HanTVIcon(
    ...
    Canvas(modifier = modifier) {
        ...
        when (icon) {
            ...
            HanTVIcon.SORT -> { // descending bars — classic sort glyph
                drawLineStroke(p(4f, 7f), p(20f, 7f), tint, stroke)
        ...
        HanTVIcon.SETTINGS -> {
            ...
            HanTVIcon.VOLUME_LOW -> {
                drawPath(speaker(::p), tint, style = Fill)
                drawArc(tint, -52f, 104f, false, topLeft = p(11.5f, 8.5f), size = Size(5f * s, 7f * s), style = stroke)
            }
            HanTVIcon.VOLUME_MUTE -> {
                drawPath(speaker(::p), tint, style = Fill)
            ...
            HanTVIcon.EXPAND -> { // ⤢ open-in-full: diagonal with arrowheads at both ends
            ...
            HanTVIcon.IMAGE -> { // photo/picture frame: rounded rect + sun + mountain
            ...
            HanTVIcon.SWAP -> { // ⇄ switch/swap engine (top arrow →, bottom arrow ←)
        ...
        HanTVIcon.SPARKLE -> {
            ...
            HanTVIcon.TEXT_SIZE -> { // Big A + small a — a typeface, not a colour.
            ...
            HanTVIcon.BACKUP -> { // Archive box: lid, body, handle. A backup is not a download.
            ...
            HanTVIcon.FOLDER -> { // Folder: the tab, then the body it sits on.
            ...
            HanTVIcon.REFRESH -> { // Circular arrow, head drawn as the usual corner marker.
            ...
            HanTVIcon.MOTION -> { // A body sweeping round, with speed lines trailing behind it.
            ...
            HanTVIcon.GLOW -> { // Light spilling from a centre: a solid core inside two pairs of arcs.
            ...
            HanTVIcon.WARNING -> { // A clock read as "history" on the error log; a triangle cannot.
            ...
            HanTVIcon.LIST_GRID -> { // Four tiles — the browsing lists themselves.
            ...
            HanTVIcon.PANEL_WIDTH -> { // Two walls with a two-headed arrow between them.
            ...
            HanTVIcon.FOCUS_HIGHLIGHT -> { // A row with light spilling off its rim — the focus ring itself.
            ...
            HanTVIcon.CH_NAV -> { // A TV screen with page arrows inside: paging channels, not skipping a track.
                drawRoundRectStroke(p(3f, 5f), p(21f, 19f), 2.5f * s, tint, stroke)
            ...
            HanTVIcon.AUTOPLAY_NEXT -> { // Play-to-bar with a loop arcing over it: it goes on by itself.
            ...
            HanTVIcon.LIVE_DOT -> { // A lit core with two pairs of arcs spilling off it — "on the air".
            ...
            HanTVIcon.MORE -> { // ⋯ — the rest of the controls, when the window is too small for them.
                drawCircle(tint, 1.7f * s, p(5.5f, 12f))
                drawCircle(tint, 1.7f * s, p(12f, 12f))
                drawCircle(tint, 1.7f * s, p(18.5f, 12f))
            }
            HanTVIcon.SEEK_BACK -> { // REFRESH's circular arrow, mirrored: jump back through the stream.
